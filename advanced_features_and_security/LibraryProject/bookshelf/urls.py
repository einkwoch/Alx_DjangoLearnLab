...
urlpatterns = [
    path('books/', list_books, name='book_list'),  # URL for function-based view
    path('library/<int:pk>/', LibraryDetailView.as_view(), name='library_detail'),  # URL for the class-based view
    path('login/', LoginView.as_view(template_name='bookshelf/login.html'), name='login'),  # Using built-in login view
    path('logout/', LogoutView.as_view(template_name='bookshelf/logout.html'), name='logout'),  # Using built-in logout view
